        ...
        if (vm.chainView) {
          newCommentsFromOthers = _.filter(newComments, function (cm) {
            return !_.some(ctrlComments(), function (sentCm, index) {
              ...
              return sentCm.type === cm.type && (sentCm.comment && cm.comment && sentCm.comment.body === cm.comment.body && vm.user.id === cm.comment.sender.id && sentCm.comment.attachmentId === cm.comment.attachmentId);
            })
          });
