(function() {
  ...
  /** @ngInject */
  function StoreService($window, $rootScope, $q, $http, $sce, $log,
                       NstSvcServer, NST_SRV_RESPONSE_STATUS, NST_SRV_ERROR, NST_STORE_UPLOAD_TYPE,
                       NstStore) {
    function Store(stores) {
      this.stores = {};
      this.defaultStore = new NstStore();

      if (angular.isArray(stores) || angular.isObject(stores)) {
        for (var k in stores) {
          this.stores[stores[k].id] = new NstStore(stores[k]);
        }
      }
    }
    ...
    Store.prototype = {
      ...
      getStore: function (sid) {
        if (!this.stores.hasOwnProperty(sid)) {
          this.stores[sid] = (new NstStore()).setData(sid).then(function (store) {
      ...
      upload: function (file, type, id, onUploadStart) {
        ...
        return this.defaultStore.getUploadToken().then(function (token) {
          ...
          return $http({
            ...
            return $q(function (res, rej) {
              switch (data.status) {
                case NST_NST_SRV_RESPONSE_STATUS.SUCCESS:
                  res(data);
                  break;
                default:
                  rej(data);
    ...
    var service = new Store(stores ? angular.fromJson(stores) : undefined);
